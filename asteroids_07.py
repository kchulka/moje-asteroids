...
- větší asteroid se rozpadne na 2-3 menší
- Asteroid.new_child_asteroid()
...
class Asteroid(SpaceObject):
    ...
    ASTEROID_IMAGE += ['meteorGrey_big2.png', 'meteorGrey_med2.png', 'meteorGrey_small2.png', 'meteorGrey_tiny2.png']
    SMALL_ASTEROID = 25

    def __init__(self, x, y, direction=0, images=ASTEROID_IMAGE, speed=None):
        img = random.choice(images)
        if speed == None:
    ...
    def new_child_asteroid(self): 
        """ vytvoří nové 2-3 asteroidy po rozpadu """
        global vesmir
        for i in range(random.randint(2, 3)):
            speed = random.randint(1, 3) + self.speed
            direction = self.rotation + random.uniform(-math.pi/4, math.pi/4)
            vesmir += Asteroid(x=self.x, y=self.y, direction=direction, speed=speed) 

    def hit_by_laser(self, laser):
        """ zasažen laserem """
        if self.radius > Asteroid.SMALL_ASTEROID:
            self.new_child_asteroid()
        self.delete()
...
class Space:
    ...
    SPACE_IMAGE = ['space1.jpg', 'space2.jpg', 'space5.png', 'space6.jpg', 'space7.jpg', 'space8.jpg']
    ...
    def new_spaceship(self):
        """ přidá novou raketu do vesmíru """
        if  len([o for o in self.objects if isinstance(o, SpaceShip)]) == 0:
            self += SpaceShip(x=Space.WIDTH//2, y=Space.HEIGHT//2)
    ...
    def tik(self, dt):
        ...
        if key.PLUS in stisknute_klavesy:
            self.new_spaceship()                # přidání rakety
    
    def new_asteroid(self, dt):
        """ přidá náhodný asteroid do vesmíru """
        xx, yy = random.choice([(0,1), (1,0)])
        x = random.randrange(Space.OKRAJ, Space.WIDTH - Space.OKRAJ) * xx
        y = random.randrange(Space.OKRAJ, Space.HEIGHT- Space.OKRAJ) * yy
        if x == 0:
            r = random.uniform(-math.pi/2 + Space.K , math.pi/2 - Space.K)
        elif y == 0:
            r = random.uniform(0 + Space.K, math.pi - Space.K)

        self += Asteroid(x, y, direction=r)
        pyglet.clock.schedule_once(self.new_asteroid, Space.TIME)
...
pyglet.clock.schedule_once(vesmir.new_asteroid, 3)
